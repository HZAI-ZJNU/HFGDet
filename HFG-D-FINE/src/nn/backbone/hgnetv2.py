...
class StemBlock(nn.Module):
    ...
    def __init__(self, in_chs, mid_chs, out_chs, use_lab=False):
        ...
        self.stem1_d = FGD(in_chs,mid_chs)
        ...
        self.stem3_d = FGD(mid_chs * 2,mid_chs)
...
class HG_Stage(nn.Module):
    def __init__(
        ...
        if downsample:
            ...
            # self.down_wt = Down_wt3_mixer3(in_chs,in_chs)
        else:
            self.downsample = nn.Identity()
            # self.down_wt = nn.Identity()
    ...
    def forward(self, x):
        x = self.downsample(x)
        # x = self.downsample(x) + self.down_wt(x)
        # x = self.down_wt(x) # 43 用的
        x = self.blocks(x)
        return x
    ...
    def __init__(
        ...
        if pretrained:
            ...
            try:
                ...
                self.load_state_dict(state,strict=False) # 用非严格模式
