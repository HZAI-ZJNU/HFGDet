...
from .hfg_module import FBAL,FGIL
...
class RepNCSPELAN4(nn.Module):
    # csp-elan       c1:256*2 c2:256 c3:256*2 c4:256/2
    ...
    def forward(self, x): # x 2c
        y = list(self.cv1(x).split((self.c, self.c), 1)) # y: c
        y.extend(m(y[-1]) for m in [self.cv2, self.cv3])
        return self.cv4(torch.cat(y, 1))
...
class HybridEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.wt_fusions = nn.ModuleList() # add
        self.mfms = nn.ModuleList()
        for idx in range(len(in_channels) - 1, 0, -1):
            self.lateral_convs.append(ConvNormLayer_fuse(hidden_dim, hidden_dim, 1, 1))
            self.wt_fusions.append(FBAL(hidden_dim, hidden_dim)) # add
            self.mfms.append(FGIL(hidden_dim,hidden_dim))
        ...
        for idx in range(len(in_channels) - 1):
            ...
            self.mfms2.append(FGIL(hidden_dim,hidden_dim))
    ...
    def forward(self, feats):
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            feat_low = proj_feats[idx - 1] # 的好好看下尺寸的问题
            feat_heigh = self.lateral_convs[len(self.in_channels) - 1 - idx](feat_heigh)
            inner_outs[0] = feat_heigh
            # upsample_feat = F.interpolate(feat_heigh, scale_factor=2.0, mode="nearest")
            upsample_feat = self.wt_fusions[len(self.in_channels) - 1 - idx]([feat_heigh,feat_low])
            fusion_feat = self.mfms[len(self.in_channels) - 1 - idx]([upsample_feat,feat_low])
            # inner_out = self.fpn_blocks[len(self.in_channels) - 1 - idx](
            #     torch.concat([upsample_feat, feat_low], dim=1)
            # )
            inner_out = self.fpn_blocks[len(self.in_channels) - 1 - idx](fusion_feat)
        ...
        for idx in range(len(self.in_channels) - 1):
            ...
            fusion_feat = self.mfms2[idx]([feat_height,downsample_feat])
            # out = self.pan_blocks[idx](torch.concat([downsample_feat, feat_height], dim=1))
            out = self.pan_blocks[idx](fusion_feat)
            outs.append(out)
