...
from .wavelet import create_wavelet_filter,inverse_wavelet_transform,wavelet_transform
...
class AdaptLocalHighFrequenceMixer3(nn.Module): # cuba
    def __init__(self, in_channels: int,
                 out_channels: int,
                 g: int = 1,):
        super().__init__()
        ...
        # self.proj = Conv(in_channels, out_channels, k=1)
...
class WT(nn.Module):
    ...
    def forward(self, x, invert=False):
        if not invert:
            ...
            re = inverse_wavelet_transform(x, self.iwt_filter)
